...
import { delAll } from "../../redux/slices/cartSlice";


const CartPage = () => {

    const { cartPizzas, pizzasAmount, priceSum } = useSelector(state => state.cart);
    const dispatch = useDispatch();

    const clearAllPizzas = () => {
        dispatch(delAll());
    }
        ...
        <div className="container container--cart">
        {
                cartPizzas.length > 0
                    ?<div className="cart">
                        ...
                        Корзина</h2>
                        <div className="cart__clear" onClick={clearAllPizzas}>
                      ...
                      <div className="content__items">
                        {
                            cartPizzas.map((pizza) => {
                                return <CartItemContainer {...pizza} />
                            })
                        }
                      </div>
                      <div className="cart__bottom">
                        <div className="cart__bottom-details">
                          <span> Всего пицц: <b>{ pizzasAmount } шт.</b> </span>
                          <span> Сумма заказа: <b>{ priceSum } ₽</b> </span>
                        </div>
